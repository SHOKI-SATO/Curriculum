...
@Entity
//自クラスがデータの入れ物となるEntityクラスであることを示す
@Table(name="accounts")
//Entityクラスと対応するデータベースのテーブル名を指定（対応するテーブル名を指定）
public class Account implements UserDetails {
    ...
    @Id
    //Entityクラス内で定義した主キーに付与
	@Column(nullable = false, unique = true)
    //Entityクラス内で定義した、テーブルのカラム名に相当するメンバ変数に付与、データ型や長さを指定することが可能（NOT NULL制約）
//    ユニーク制約・・同じ値入れられない
...
//	有効か
	...
	//時刻を表す型（java.util.Dateおよびjava.util.Calendar）を持つ永続化プロパティまたは永続化フィールドに指定
	private Date createdAt;

	// roleは複数管理できるように、Set<>で定義。
	@ElementCollection(fetch = FetchType.EAGER)
//	CollectionをEntityの属性として使用する
	@Enumerated(EnumType.STRING)
//	Enum の値を DB に格納することを宣言　列挙型をマッピングするのに使われる型を指定
	@Column(nullable = false)
	...
	//登録時に、日時を自動セットする
	@PrePersist
//	データベースにINSERT文を発行する前に呼び出されるコールバックメソッドであることを示す
